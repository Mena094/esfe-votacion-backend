...
const createItem = async ({ Nombre, IdCategoria }) => {
  try {
    const [exist] = await pool.query("SELECT * FROM Participante WHERE Nombre = ?", [Nombre]);
    if (exist.length > 0) {
      return -1; // Participante con el mismo nombre ya existe
    }
    const [concurso] = await pool.query("SELECT * FROM Categoria WHERE Id = ?", [IdCategoria]);
    if (concurso.length < 1) {
      return -2; // 
    }
    const query = "INSERT INTO Participante (Nombre, IdCategoria) VALUES (?, ?)";
...
const updateItem = async (body) => {
  try {
    ...
    const query = "UPDATE Participante SET Nombre = ?, IdCategoria = ? WHERE Id = ?";
